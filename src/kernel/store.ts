...
export class SqliteRunStore {
  ...
  private validate(run: Run, id: string, revision: number): void {
    ...
    if (run.tickets.some((value) => typeof value.id !== "string" || typeof value.title !== "string" || !ticketStatuses.has(value.status) || !Array.isArray(value.blockedBy) || value.blockedBy.some((blocker) => typeof blocker !== "string") || !Array.isArray(value.evidence) || value.evidence.some((ref) => !artifact(ref)) || (value.commitId !== undefined && !/^[a-f0-9]{40,64}$/.test(value.commitId)))) throw new CrustError("TICKET_TAMPERED", `Run ${id} ticket failed schema validation`);
