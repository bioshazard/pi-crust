import { CrustError, type ArtifactRef, type ProposalKind, type Projection, type Run, type State, type Ticket } from "../kernel/types.js";
...
const TERMINALS: Record<State, string> = {
  ...
  REVIEWING: "Propose separate standards and specification findings.",
...
export class PocockClient {
  readonly workflowRevision = "pocock-v1.1-poc-1";
  readonly phaseRevision = "pocock-phases-1";
  readonly projectionRevision = "pocock-projection-1";
  readonly guardRevision = "pocock-guards-1";
  ...
  validate(run: Run, payload: unknown): void {
    ...
    switch (this.proposalKind(run)) {
      ...
      case "ticket_ready_for_review":
        ...
        return;
      case "review": requireArray(payload, "standardsFindings"); requireArray(payload, "specificationFindings"); return;
  ...
  accept(run: Run, payload: unknown): { from: State; to: State } {
    ...
    switch (run.state) {
      ...
      case "SLICING":
        run.tickets = (record.tickets as Array<{ id: string; title: string; blockedBy: string[] }>).map((ticket) => ({ ...ticket, status: "pending", evidence: [] }));
  ...
  private validateTickets(values: unknown[]): void {
    ...
    const tickets = values.map((value) => {
      if (!isRecord(value) || typeof value.id !== "string" || typeof value.title !== "string" || !Array.isArray(value.blockedBy) || !value.blockedBy.every((id) => typeof id === "string")) throw new CrustError("INVALID_PROPOSAL", "Each ticket needs id, title, and blockedBy");
      return value as { id: string; title: string; blockedBy: string[] };
    });
